...
export default function PublisherNews() {
  ...
  const authorKey = (user?.email || user?.displayName || '').trim()
  ...
  async function load() {
    ...
    try {
      const [news, categories] = await Promise.all([
        adminService.newsByAuthor(authorKey, token),
      ...
      toast.error(e?.response?.data?.message || e?.data?.message || 'Не вдалося завантажити дані')
    } finally {
      setLoading(false)
    }
  }

  React.useEffect(() => { load() }, [authorKey])
  ...
  return (
    <div className="ns-card ns-shadow p-4">
      <div className="d-flex justify-content-between align-items-end mb-3">
        <div>
          <h1 className="h4 fw-semibold mb-1">Мої новини</h1>
          <div className="ns-muted small">Створення новин без ручного вводу автора (береться з профілю/токена).</div>
        </div>
        <div className="d-flex gap-2">
          ...
          <button
            ...
            onClick={() => setEditing({ id: 0, title: '', content: '', category: 0, imageUrls: [] })}
          >
      ...
      {items.length === 0 ? (
        <EmptyState title="Поки немає ваших новин" hint="Натисніть “Додати”, щоб створити першу публікацію." />
      ) : (
            ...
            <tbody>
              {items.map(n => (
                <tr key={n.id}>
                  ...
                  <td className="text-truncate" style={{ maxWidth: 560 }}>{n.title}</td>
                  <td>
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      ...
                      <button className="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target={`#del-pub-${n.id}`}>
...
function EditPublisherNewsModal({ value, onClose, onSaved, token, categories, author }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || e?.data?.message || e?.response?.data?.title || 'Не вдалося зберегти')
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="modal-content bg-dark text-light border border-secondary">
            ...
            <div className="modal-body">
              ...
              <div className="mb-2">
                ...
                <input className="form-control ns-input" value={author || ''} disabled />
                <div className="ns-muted small mt-1">Автор підтягується автоматично.</div>
              </div>
